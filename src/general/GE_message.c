...
void DS_message_set(char *input, ...)
{
	...
	va_list va;
	va_start(va, input);
	if(ds_message_set(message, input, &va))
		state = 1;
	va_end(va);

	ds_write_to_string(0, store, message);
}
...
void DS_message_append(char *input, ...)
{
	va_list va;
	va_start(va, input);
	ds_message_set(message, input, &va);
	va_end(va);
...
void DS_message_insert(char *input, ...)
{
	...
	ds_write_to_string(1, store, message);
	state = 1;
}
...
{
	ds_write_to_string(1, store, "info: ");
	strcpy(send, store);
	store[0] = '\0';
	state = 0;
#ifdef _unix_
